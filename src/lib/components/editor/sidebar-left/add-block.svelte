											...
											<Sidebar.MenuSubItem>
												<Sidebar.MenuSubButton>
													{#snippet child({ props })}
														<button
															{...props}
															class="flex h-full w-full cursor-grab flex-col items-center justify-center rounded-md border bg-background p-2 py-4 active:cursor-grabbing"
															draggable="true"
															ondragstart={handleDragStart(block.id, block.name)}
															ondragend={handleDragEnd}
														>
															<block.icon class="mb-2 size-4" />
